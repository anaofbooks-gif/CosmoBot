        ...
        try:
            ...
            for m in metas:
                ...
                if not data_valida(data_meta) or not texto_meta:
                    continue
                dados["lembretes_metas"].append({"data": data_meta, "livro": livro_txt, "autor": autor, "meta": texto_meta, "canal_id": topico.id, "thread_id": topico.id, "avisado": False, "tipo": "lc"})
                criados += 1
            guardar_dados()
            if Image:
                try:
...
"""
        try:
            ...
            for m in metas:
                ...
                if not data_valida(data_meta) or not texto_meta:
                    continue
                dados["lembretes_metas"].append({"data": data_meta, "livro": livro_txt, "autor": autor, "meta": texto_meta, "canal_id": canal_id, "thread_id": canal_id, "avisado": False, "tipo": "lc"})
                criados += 1
            guardar_dados()
            if Image:
                try:
