...
class StatsCog(commands.Cog):
    """Comandos de estatísticas e resumos"""

    def __init__(self, bot):
        self.bot = bot

    @commands.command(name="resumomes", help="Gera gráfico circular das leituras de um mês.")
    async def resumo_mensal(self, ctx: commands.Context, mes: Optional[str] = None):
        if Image is None:
    ...
    @commands.command(name="resumoano", help="Apresentação visual do ano de leituras.")
    async def resumo_anual(self, ctx: commands.Context, ano: Optional[int] = None):
        if Image is None:
            return await ctx.send("❌ Falta instalar Pillow: `pip install Pillow`")
