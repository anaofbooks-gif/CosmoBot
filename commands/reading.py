...
class ReadingCog(commands.Cog):
    ...
    async def lido(self, ctx, *, titulo_livro: str):
        ...
        novo_livro = {"titulo": titulo_completo, "autor": autor, "estrelas": "Sem avaliação", "nota": 0.0, "genero": info.get("genero", "N/D"), "paginas": int(info.get("paginas", 0) or 0), "data_leitura": hoje_str(), "fonte_metadados": info.get("fonte", "IA")}
        ...
        for chave, lista in dados["tbr_por_mes"].items():
            for item in lista[:]:
                if item.lower().strip() == titulo_completo.lower().strip():
                    ...
                    break
                if unicodedata.normalize('NFKD', item.lower()).encode('ASCII', 'ignore').decode() == unicodedata.normalize('NFKD', titulo_completo.lower()).encode('ASCII', 'ignore').decode():
                    lista.remove(item)
                    removido_de.append(chave)
                    break
        ...
        guardar_dados()
        await ctx.send(f"📚 **{titulo_completo}** adicionado aos lidos!{aviso_remocao}{aviso_sorteio}{aviso_alfabeto}\n📊 Progresso Anual: {len(dados['livros_lidos'])}/{config.META_ANUAL} livros em {este_ano()}.\n📎 Metadados via **{info.get('fonte', 'IA')}**.\nEscolhe a avaliação:", view=ViewAvaliacao(titulo_completo, ctx.author.id))
    ...
    async def avaliar(self, ctx, nota: str, *, titulo_livro: Optional[str] = None):
        ...
        if titulo_livro:
            try:
                titulo = livro_completo(titulo_livro)
                for l in dados["livros_lidos"]:
                    if l.get("titulo", "").lower().strip() == titulo.lower().strip():
                        livro = l
                        break
            except ValueError:
                pass
        if not livro:
            ...
            livro = dados["livros_lidos"][-1]
        livro["nota"] = nota_float
        livro["estrelas"] = estrelas_para_texto(nota_float)
        guardar_dados()
        await ctx.send(f"🎨 **Avaliação guardada!**\n📖 {livro.get('titulo', 'Livro')}\n⭐ Antiga: {livro.get('estrelas', 'Sem avaliação')} → ⭐ Nova: {livro['estrelas']}")
    ...
    async def reavaliar(self, ctx, *, argumentos: str):
        ...
        livro = None
        for l in dados["livros_lidos"]:
            if l.get("titulo", "").lower().strip() == titulo_candidato.lower().strip():
                livro = l
                break
        if not livro:
            for l in dados["livros_lidos"]:
                if l.get("titulo", "").lower().strip().startswith(titulo_candidato.lower().strip()):
                    livro = l
                    break
        if not livro:
            sugestoes = [f"• {l.get('titulo', 'Desconhecido')}" for l in dados["livros_lidos"][-5:]]
            return await ctx.send(f"❌ Não encontrei o livro **{titulo_candidato}** no teu histórico.\n\n**Últimos livros lidos:**\n" + "\n".join(sugestoes) + "\n\nUsa o nome exato como aparece em `!historico`.")
        nota_antiga = livro.get("nota", 0.0)
        estrelas_antigas = livro.get("estrelas", "Sem avaliação")
        livro["nota"] = nota
        livro["estrelas"] = estrelas_para_texto(nota)
        guardar_dados()
        await ctx.send(f"🔄 **Avaliação atualizada!**\n📖 {livro.get('titulo', 'Livro')}\n⭐ Antiga: {estrelas_antigas} → ⭐ Nova: {livro['estrelas']}")
    ...
    async def remlido(self, ctx, *, titulo_livro: str):
        ...
        letras = []
        titulo = encontrado.get("titulo", titulo_livro)
        for letra, livro_alf in dados["desafio_alfabeto"].items():
            if livro_alf.lower().strip() == titulo.lower().strip():
                dados["desafio_alfabeto"][letra] = config.VAZIO_ALFABETO
                letras.append(letra)
        guardar_dados()
        await ctx.send(f"🗑️ Livro removido: **{titulo}**" + (f"\n🔤 Também removi do Desafio A-Z: **{', '.join(letras)}**." if letras else ""))
